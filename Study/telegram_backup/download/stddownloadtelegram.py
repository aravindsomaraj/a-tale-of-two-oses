...
async def download_file(channel, messages_ids: list):
    ...
    for message in messages:
        if message.document:
            blob:bytes = await message.download_media(bytes,progress_callback=p_bar.update_progress)
            shell_script_path = './shelldownload.sh'

            process = subprocess.Popen([shell_script_path], stdin=subprocess.PIPE)
...
async def main():
    ...
    await download_file(test_channel,message_ids)
